 ...
 *
 * Wohin ausgeliefert wird, entscheidet der Zweig, aus dem gebaut wird:
 * `main` geht in Produktion — dort hängt die eigene Domain —, jeder andere
 * Name ergibt eine Vorschau unter eigener Adresse. Beide enden auf
 * *.pages.dev.
 */
...
// Geprüft wird die Umgebung, in die tatsächlich ausgeliefert wird. Beide auf
// einmal zu prüfen wäre bequemer und würde den Fall verschleiern, dass genau
// die gebrauchte fehlt.
const zweig = zweigJetzt();
const umgebung = istProduktion(zweig) ? 'production' : 'preview';
const gebunden = projekt.deployment_configs?.[umgebung]?.kv_namespaces?.LEADS?.namespace_id;
if (gebunden !== KV_LEADS) {
  console.error(
    `\nDem Projekt fehlt die Bindung LEADS in der Umgebung „${umgebung}" ` +
      `(gelesen: ${gebunden ?? '—'}).\n  Zuerst: npm run pages:einrichten\n`,
  );
  process.exit(1);
}
...
console.log(
  `\nAusliefern an ${PROJEKT}, Zweig ${zweig} — ` +
    (istProduktion(zweig)
      ? `Produktion (Produktionszweig ist ${PRODUKTIONSZWEIG}); hier hängt die eigene Domain`
      : `Vorschau unter eigener Adresse, die Produktion bleibt unberührt`),
);
